...
import { XError } from 'xerror';
	...
	export class Client {
        ...
        static async start(config: Partial<Config>): Promise<Client> {
            return new Promise((resolve, reject) => {
                const tight_client = new Client(config);
                window.api.invoke<Partial<Config>,number>('StartTightCNC',tight_client.config).then((pid) => {
                    console.log('PID', pid);
                    /*
